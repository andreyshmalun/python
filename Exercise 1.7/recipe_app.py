...
def create_recipe():
    # Set name and cooking_time variables
    name, cooking_time = None, None

    name_input = str(
        input('Enter the name of the recipe (LETTERS ONLY, max of 50 char.):   '))
    if len(name_input) < 51 and name_input.isalpha():
        # Get user defined name and set to name
        name = name_input
    else:
    ...
    if cooking_time_input.isnumeric():
        ...
        # Return to main menu if user entered cooking time is not valid
        return None
...
def view_all_recipes():
    # Get all recipe object from database
    recipes_list = session.query(Recipe).all()
    if recipes_list == []:
        print("\n There are currently no recipes in the database!")
        # If there're no recipes on database, notify user and return to main menu
        return None
    else:
        for recipe in recipes_list:
            # Display string representation of each recipe object in database
            print(recipe)
...
def delete_recipe():
    ...
    if recipes_count == 0:
        ...
        if id_picked.isnumeric():
            ...
            if id_picked in recipe_ID_available:
                recipe_to_delete = session.query(
                    Recipe).filter(Recipe.id == id_picked).one()
                print("\n Are you sure you want to delete " +
                      recipe_to_delete.name + " recipe. This action cannot be undone!")
                sure_or_not = input(
                    "Enter 'Yes' if you're sure otherwise 'No':   ")
                ...
                if sure_or_not == 'no':
                    ...
                    recipe_to_be_deleted = session.query(Recipe).filter(
                        Recipe.name == recipe_to_delete.name).one()
